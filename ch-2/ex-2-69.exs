...
defmodule MySet do
  ...
  def encode_symbol(value, tree) do
    if is_nil(tree) do
      ...
      cond do
        left == :leaf || right == :leaf -> nil
        symbol_leaf(left) == value ->
  ...
  def generate_huffman_tree(pairs) do
    # Reverse the make_leaf_set results first
    # successive_merge(reverse(make_leaf_set(pairs), nil))
    successive_merge(make_leaf_set(pairs))
  end

  def reverse(set, acc) do
    cond do
      is_nil(set) -> acc
      true ->
        reverse(cdr(set), cons(car(set), acc))
    end
  end
  ...
  # def successive_merge(leaf_set) do
  #   cond do
  #     is_nil(cdr(cdr(leaf_set))) ->
  #       MySet.make_code_tree(
  #         car(leaf_set), cadr(leaf_set)
  #       )
  #     true ->
  #       MySet.make_code_tree(
  #         car(leaf_set),
  #         successive_merge(cdr(leaf_set))
  #       )
...
MySet.generate_huffman_tree(pairs) |> MySet.puts
